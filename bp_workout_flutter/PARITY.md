...
**Goal:** Same product behavior as `bp-workout` (Swift + SwiftData + Blueprint API auth).
...
| Feature | iOS | Flutter |
|---------|-----|---------|
| Config: `BLUEPRINT_API_URL` | `AuthConfig` | **Done** (`env.dart` + dart-define) |
| Sign in / sign up / forgot password | `AuthFlowViews` | **Done** (`AuthService` → `/v1/auth/*`) |
| Saved email | `supabase.saved.email` | **Done** (`AuthConstants.savedEmailKey`) |
| Secure session storage | `KeychainStore` | **Done** (`flutter_secure_storage`) |
...
| `POST /v1/workouts` sync | `BlueprintWorkoutSyncClient` | **Partial** (Bearer + same JSON shape; errors surfaced if offline) |
