# Legacy Postgres migrations (Blueprint Workout)

> **Production** uses **Railway Postgres** — see [`db/railway/MIGRATION.md`](../db/railway/MIGRATION.md) and [`db/railway/schema.sql`](../db/railway/schema.sql). This folder keeps historical Supabase CLI migrations and seed scripts for reference and one-off data generation.

Schema was originally managed with the **Supabase CLI** (`migrations/`, `config.toml`). **Bundled catalog + reference progress** are loaded by migration `20260412210000_seed_bundled_catalog_and_progress.sql`, generated by `supabase/scripts/generate_seed.py` from **`supabase/source/workout_programs.json`** and **`bp-workout/Resources/progress_data.json`**. Clients read the **program catalog** from the **Blueprint API**.

**AI / LLM:** OpenAI is **not** called from mobile apps. Run the **`api/`** service (see `api/README.md`); it uses **JWT auth** and **Postgres** on Railway.
...
Configure **Site URL** and **Redirect URLs** for password recovery when SMTP is wired on the API. App uses `bpworkout://auth` deep link for reset completion.
